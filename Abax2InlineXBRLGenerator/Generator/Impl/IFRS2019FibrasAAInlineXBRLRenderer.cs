...
/// </summary>
public class IFRS2019FibrasAAInlineXBRLRenderer : BaseInlineXBRLRenderer
{
    ...
    protected override TemplateConfiguration FillDocumentConfig(XbrlFactFinder factFinder, XBRLInstanceDocument instanceDocument)
    {
        ...
        instanceDocument.DocumentConfig.DateOfEndOfReportingPeriod = factDateOfEnd?.Value ?? "N/A"; instanceDocument.DocumentConfig.FullTitle = (entityName ?? "N/A") + " - " + periodString;
        ...
        var minimumPeriodDate = instanceDocument.Contexts.Values.Where(x => x.Period != null).Min(x => x.Period.PeriodInstantDate != null ? DateTime.Parse(x.Period.PeriodInstantDate) : DateTime.Parse(x.Period.PeriodEndDate));
        instanceDocument.DocumentConfig.DocumentVariables.Add("entityIncorporationDate", minimumPeriodDate.ToString(Constants.XBRLPeriodDateFormat));
        instanceDocument.DocumentConfig.EntityIncorporationDate = minimumPeriodDate.ToString(Constants.XBRLPeriodDateFormat);
